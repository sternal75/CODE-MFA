...
%   case it should be zero
    ...
    for j=1:size(sample_mat,1)
        ...
        % if the variance was specified as input, take its value
        met_list_norm{i}.var(find(~isnan(v_input)), j) = v_input(find(~isnan(v_input)));
